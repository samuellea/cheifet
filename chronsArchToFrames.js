...
function processArrayOfObjects(arr) {
  ...
  const updatedArray = arr.map((obj) => {
    // Check if .internetArchiveURL exists, has elements, and doesn't end with '.mpeg'
    if (
      obj.internetArchiveURL &&
      obj.internetArchiveURL.length > 0 &&
      !obj.internetArchiveURL[0].endsWith('.mpeg')
    ) {
      // Check if .framesURL is an empty array, null, or an array containing null
      if (
        obj.framesURL === null ||
        obj.framesURL === undefined ||
        (Array.isArray(obj.framesURL) &&
          obj.framesURL.every((item) => item === null))
      ) {
        // Extract the portion after 'details/' from the first element of .internetArchiveURL
        const urlPart = obj.internetArchiveURL[0].split('details/')[1];

        // Construct the new URL
        const newUrl = `https://api.gdeltproject.org/api/v2/tvv/tvv?id=IAITEM_${urlPart}`;

        // Set the .framesURL to the new URL in an array
        obj.framesURL = [newUrl];
      }
    }
  ...
  // Write the updated array to 'allArchsAndFramesAccountedFor.js'
  fs.writeFile(
    'allArchsAndFramesAccountedFor.js',
...
processArrayOfObjects(chronsEpisodes);
